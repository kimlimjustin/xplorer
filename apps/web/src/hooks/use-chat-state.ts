// Mock useChatState hook for web landing page demos
...
function chatReducer(state: ChatState, action: ChatAction): ChatState {
  switch (action.type) {
    ...
    case 'SET_OLLAMA_STATUS': return { ...state, ollamaStatus: action.status };
    case 'SET_MODEL_DROPDOWN_OPEN': return { ...state, isModelDropdownOpen: action.open };
    case 'SET_CONTEXT_DROPDOWN_OPEN': return { ...state, isContextDropdownOpen: action.open };
    case 'SET_SETTINGS_MINIMIZED': return { ...state, isSettingsMinimized: action.minimized };
    case 'SET_CONTEXT_SEARCH_QUERY': return { ...state, contextSearchQuery: action.query };
    case 'SET_CONTEXT_FILES': return { ...state, contextFiles: action.files };
    ...
    case 'REMOVE_CONTEXT_FILE': return { ...state, contextFiles: state.contextFiles.filter(f => f.path !== action.path) };
    ...
    case 'UPDATE_TOOL_CALL': return { ...state, toolCalls: state.toolCalls.map(t => t.id === action.id ? { ...t, ...action.updates } : t) };
    case 'TOGGLE_TOOL_CALL_EXPAND': return { ...state, toolCalls: state.toolCalls.map(t => t.id === action.id ? { ...t, expanded: !t.expanded } : t) };
    case 'SET_PENDING_APPROVALS': return { ...state, pendingApprovals: action.approvals };
    ...
    case 'AGENT_SEND_COMPLETE': return { ...state, streamingText: '', isAgentRunning: false, activePlan: null };
    case 'AGENT_SEND_ERROR': return { ...state, streamingText: '', isAgentRunning: false, activePlan: null };
    case 'RESET_CONTEXT': return { ...state, contextFiles: [], includeCurrentFolder: true };
...
export function useChatState() {
  ...
  const setAvailableModels = useCallback((models: AIModel[]) => dispatch({ type: 'SET_AVAILABLE_MODELS', models }), []);
  const setSelectedModel = useCallback((model: string) => dispatch({ type: 'SET_SELECTED_MODEL', model }), []);
  const setOllamaStatus = useCallback((status: boolean) => dispatch({ type: 'SET_OLLAMA_STATUS', status }), []);
  const setIsModelDropdownOpen = useCallback((open: boolean) => dispatch({ type: 'SET_MODEL_DROPDOWN_OPEN', open }), []);
  const setIsContextDropdownOpen = useCallback((open: boolean) => dispatch({ type: 'SET_CONTEXT_DROPDOWN_OPEN', open }), []);
  const setIsSettingsMinimized = useCallback((minimized: boolean) => dispatch({ type: 'SET_SETTINGS_MINIMIZED', minimized }), []);
  const setContextSearchQuery = useCallback((query: string) => dispatch({ type: 'SET_CONTEXT_SEARCH_QUERY', query }), []);
  const setContextFiles = useCallback((files: FileContext[]) => dispatch({ type: 'SET_CONTEXT_FILES', files }), []);
  const addContextFile = useCallback((file: FileContext) => dispatch({ type: 'ADD_CONTEXT_FILE', file }), []);
  const removeContextFile = useCallback((path: string) => dispatch({ type: 'REMOVE_CONTEXT_FILE', path }), []);
  ...
  const updateToolCall = useCallback((id: string, updates: Partial<ToolCallDisplay>) => dispatch({ type: 'UPDATE_TOOL_CALL', id, updates }), []);
  const addPendingApproval = useCallback((approval: AgentToolCall) => dispatch({ type: 'ADD_PENDING_APPROVAL', approval }), []);
  const removePendingApproval = useCallback((id: string) => dispatch({ type: 'REMOVE_PENDING_APPROVAL', id }), []);
