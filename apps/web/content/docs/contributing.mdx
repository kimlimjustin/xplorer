...
1. Edit MDX files in `apps/web/content/docs/`
2. Update sidebar navigation in `apps/web/src/lib/docs-sidebar.ts` if adding new pages
3. Run `cd apps/web && pnpm dev` to preview locally
...
export const TauriAPI = {
  // ... existing methods
  myNewCommand: (param: string): Promise<MyResult> => invoke('my_new_command', { param }),
};
```
