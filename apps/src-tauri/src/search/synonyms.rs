...
pub static SYNONYMS: LazyLock<HashMap<&'static str, &'static [&'static str]>> = LazyLock::new(|| {
    let mut m: HashMap<&'static str, &'static [&'static str]> = HashMap::new();
        ...
        m.insert("report",        &["summary", "analysis", "review", "assessment", "brief"]);
        m.insert("readme",        &["readme", "read_me", "about", "info", "information"]);
        m.insert("note",          &["notes", "memo", "jot", "annotation", "remark"]);
        m.insert("draft",         &["wip", "work_in_progress", "unfinished", "preliminary", "sketch"]);
        m.insert("letter",        &["correspondence", "mail", "message", "epistle"]);
        m.insert("memo",          &["memorandum", "note", "bulletin", "notice"]);
        m.insert("manual",        &["handbook", "guide", "reference", "documentation", "instructions"]);
        m.insert("guide",         &["tutorial", "howto", "walkthrough", "handbook", "manual"]);
        m.insert("tutorial",      &["lesson", "guide", "howto", "course", "walkthrough"]);
        m.insert("summary",       &["abstract", "overview", "synopsis", "brief", "recap"]);
        m.insert("presentation",  &["slides", "ppt", "deck", "slideshow", "keynote"]);
        m.insert("spreadsheet",   &["excel", "xls", "xlsx", "csv", "worksheet", "workbook"]);
        m.insert("invoice",       &["bill", "receipt", "statement", "charge", "tab"]);
        ...
        // Media (11 families)
        // ===================================================================
        m.insert("photo",         &["image", "picture", "pic", "snapshot", "photograph", "img"]);
        m.insert("video",         &["movie", "film", "clip", "recording", "footage", "stream"]);
        m.insert("audio",         &["sound", "music", "track", "song", "recording", "podcast"]);
        m.insert("icon",          &["favicon", "symbol", "glyph", "badge", "emblem"]);
        m.insert("animation",     &["gif", "animated", "motion", "sprite", "lottie"]);
        m.insert("thumbnail",     &["thumb", "preview", "miniature", "tile"]);
        m.insert("wallpaper",     &["background", "backdrop", "desktop", "bg"]);
        m.insert("screenshot",    &["screencap", "capture", "screen_grab", "snap", "printscreen"]);
        m.insert("diagram",       &["schematic", "flowchart", "blueprint", "sketch", "illustration"]);
        m.insert("chart",         &["graph", "plot", "visualization", "figure", "infographic"]);
        m.insert("render",        &["output", "export", "composition", "frame"]);

        // ===================================================================
        // Code / Dev (16 families)
        // ===================================================================
        m.insert("code",          &["source", "src", "script", "program", "implementation"]);
        m.insert("test",          &["spec", "unittest", "test_case", "assert", "check", "verification"]);
        m.insert("config",        &["configuration", "settings", "preferences", "options", "ini", "env", "dotfile"]);
        m.insert("build",         &["compile", "make", "bundle", "assemble", "package"]);
        m.insert("deploy",        &["release", "publish", "ship", "rollout", "launch"]);
        m.insert("library",       &["lib", "package", "crate", "module", "dependency", "gem", "jar"]);
        m.insert("template",      &["boilerplate", "scaffold", "skeleton", "starter", "blueprint"]);
        m.insert("api",           &["endpoint", "interface", "route", "service", "rest", "graphql"]);
        m.insert("database",      &["db", "datastore", "sql", "nosql", "schema", "table"]);
        m.insert("server",        &["backend", "host", "daemon", "node", "instance"]);
        m.insert("client",        &["frontend", "ui", "browser", "consumer", "app"]);
        m.insert("component",     &["widget", "element", "control", "part", "block"]);
        m.insert("module",        &["mod", "unit", "package", "chunk", "segment"]);
        m.insert("function",      &["func", "fn", "method", "procedure", "routine", "subroutine", "handler"]);
        m.insert("variable",      &["var", "param", "parameter", "argument", "field", "property", "prop"]);
        m.insert("error",         &["bug", "issue", "defect", "fault", "exception", "failure", "crash"]);

        // ===================================================================
        // File Operations (9 families)
        // ===================================================================
        m.insert("delete",        &["remove", "erase", "trash", "discard", "purge", "destroy", "unlink"]);
        ...
        m.insert("compress",      &["zip", "archive", "pack", "gzip", "deflate", "squash"]);
        m.insert("extract",       &["unzip", "decompress", "unpack", "expand", "inflate"]);
        m.insert("merge",         &["combine", "join", "concat", "concatenate", "unify"]);
        m.insert("split",         &["divide", "separate", "partition", "chunk", "slice"]);
        m.insert("create",        &["new", "add", "make", "generate", "init", "initialize"]);

        // ===================================================================
        // Data / Storage (10 families)
        // ===================================================================
        m.insert("data",          &["dataset", "information", "content", "payload", "records"]);
        m.insert("log",           &["logs", "journal", "history", "audit", "trace", "event"]);
        m.insert("backup",        &["bak", "snapshot", "checkpoint", "archive", "save"]);
        m.insert("cache",         &["cached", "tmp", "buffer", "store", "stash"]);
        m.insert("temp",          &["temporary", "tmp", "scratch", "transient", "ephemeral"]);
        ...
        m.insert("storage",       &["disk", "drive", "volume", "partition", "filesystem", "fs"]);

        // ===================================================================
        // Descriptors (12 families)
        // ===================================================================
        m.insert("old",           &["legacy", "outdated", "deprecated", "obsolete", "ancient", "vintage"]);
        m.insert("new",           &["recent", "latest", "fresh", "modern", "current", "updated"]);
        m.insert("large",         &["big", "huge", "massive", "oversized", "gigantic", "heavy"]);
        m.insert("small",         &["tiny", "mini", "compact", "little", "lightweight", "minimal"]);
        m.insert("important",     &["critical", "essential", "key", "vital", "priority", "urgent", "significant"]);
        m.insert("final",         &["complete", "finished", "done", "released", "stable", "production"]);
        m.insert("public",        &["shared", "open", "published", "accessible", "visible"]);
        m.insert("private",       &["secret", "hidden", "confidential", "restricted", "internal"]);
        m.insert("encrypted",     &["secure", "protected", "locked", "cipher", "encoded"]);
        m.insert("broken",        &["corrupt", "damaged", "invalid", "malformed", "bad"]);
        m.insert("empty",         &["blank", "vacant", "void", "null", "zero", "bare"]);
        m.insert("favorite",      &["starred", "bookmarked", "pinned", "saved", "liked"]);

        // ===================================================================
        // System (9 families)
        // ===================================================================
        m.insert("system",        &["os", "platform", "environment", "runtime"]);
        m.insert("network",       &["net", "internet", "lan", "wifi", "ethernet", "connection"]);
        m.insert("driver",        &["device_driver", "hardware", "peripheral", "adapter"]);
        m.insert("process",       &["task", "thread", "worker", "job", "pid"]);
        m.insert("service",       &["daemon", "agent", "microservice", "background"]);
        ...
        m.insert("permission",    &["access", "acl", "privilege", "rights", "auth", "authorization"]);
...
mod tests {
    ...
    fn document_returns_expected_synonyms() {
        ...
        assert!(syns.contains(&"doc"),   "expected 'doc' in synonyms for 'document'");
        assert!(syns.contains(&"file"),  "expected 'file' in synonyms for 'document'");
        assert!(syns.contains(&"text"),  "expected 'text' in synonyms for 'document'");
        assert!(syns.contains(&"paper"), "expected 'paper' in synonyms for 'document'");
    }

    #[test]
    fn photo_returns_expected_synonyms() {
        let syns = get_synonyms("photo");
        assert!(syns.contains(&"image"),     "expected 'image' in synonyms for 'photo'");
        assert!(syns.contains(&"picture"),   "expected 'picture' in synonyms for 'photo'");
        assert!(syns.contains(&"snapshot"),  "expected 'snapshot' in synonyms for 'photo'");
        assert!(syns.contains(&"img"),       "expected 'img' in synonyms for 'photo'");
    }
    ...
    fn expand_query_database_and_config() {
        ...
        assert_eq!(set.len(), expanded.len(), "expand_query produced duplicates");
    }
    ...
    fn unknown_word_returns_empty() {
        ...
        assert!(related.is_empty(), "expected empty vec for unknown word in get_all_related");
    }
