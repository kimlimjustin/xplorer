...
pub async fn resize_image(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let (new_w, new_h) =
            compute_resize_dimensions(orig_w, orig_h, width, height, maintain_aspect);
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let _ = app.emit(
            ...
            BatchImageProgress {
                ...
                status: "Starting batch processing".to_string(),
            },
        );
        ...
        let operations_ref = &operations;
        let output_dir_ref = &output_dir;

        let results: Vec<BatchImageResult> = paths
            .par_iter()
            .map(|file_path| {
                let original_size = fs::metadata(file_path).map(|m| m.len()).unwrap_or(0);
                let result = process_single_image(file_path, operations_ref, output_dir_ref);
                ...
                // Emit progress (best-effort from parallel threads)
                let _ = app.emit(
                    "batch-image-progress",
                    BatchImageProgress {
                        ...
                        processed: done,
                        total,
                        progress_percentage: (done as f64 / total as f64) * 100.0,
                        status: format!("Processed {}", file_name),
                    },
                );
