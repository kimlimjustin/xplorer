...
impl AcceleratedFileOps {
    ...
    fn supports_memory_mapping(&self) -> bool {
        ...
        match std::fs::File::open(std::env::current_exe().unwrap_or_default()) {
            Ok(file) => {
                unsafe { MmapOptions::new().len(1024).map(&file) }.is_ok()
            },
            Err(_) => false,
        }
    }
...
pub async fn accelerated_copy_file(
    ...
    thread::spawn(move || {
        ...
        let strategies_to_try = match strategy {
            ...
            CopyStrategy::BufferedSIMD => vec![
                CopyStrategy::BufferedSIMD,
                CopyStrategy::StandardBuffered,
            ],
            CopyStrategy::StandardBuffered => vec![
                CopyStrategy::StandardBuffered,
            ],
        };
        ...
        for (attempt, strategy) in strategies_to_try.iter().enumerate() {
            ...
            let result = match strategy {
                CopyStrategy::MemoryMapped => {
                    memory_mapped_copy(&source_clone, &destination_clone, &progress_manager_ref, &operation_id_clone)
                        .or_else(|e| {
                ...
                CopyStrategy::ParallelChunked => {
                    parallel_chunked_copy(&source_clone, &destination_clone, &progress_manager_ref, &operation_id_clone, &accelerated_ops.hardware_info)
                        .or_else(|e| {
                ...
                CopyStrategy::BufferedSIMD => {
                    simd_buffered_copy(&source_clone, &destination_clone, &progress_manager_ref, &operation_id_clone)
                        .or_else(|e| {
                ...
                CopyStrategy::StandardBuffered => {
                    optimized_buffered_copy(&source_clone, &destination_clone, &progress_manager_ref, &operation_id_clone)
                        .or_else(|e| {
            ...
            match result {
                ...
                Err(e) => {
                    if attempt == strategies_to_try.len() - 1 {
                        ...
                        progress_manager_ref.fail_file_operation(&operation_id_clone, format!("All copy strategies failed. Last error: {}", e));
                    }
                    continue;
                }
            }
...
fn memory_mapped_copy(
    ...
    for (chunk_idx, chunk) in mmap.chunks(MMAP_CHUNK_SIZE).enumerate() {
        ...
        let speed = if chunk_idx > 0 { bytes_processed as f64 / (chunk_idx + 1) as f64 } else { 0.0 };
        
        progress_manager.update_file_progress(
            operation_id,
            format!("Memory-mapped copying chunk {}/{}", chunk_idx + 1, total_chunks),
            0,
            bytes_processed,
            Some("MemoryMapped".to_string()),
            true,
...
fn parallel_chunked_copy(
    ...
    thread::spawn(move || {
        ...
        while let Ok(_) = progress_rx.recv() {
            ...
            let speed = if elapsed_secs > 0.0 { processed as f64 / elapsed_secs } else { 0.0 };
            
            progress_manager_clone.update_file_progress(
                &operation_id_clone,
                format!("Parallel copy: {:.1}%", (processed as f64 / total_bytes as f64) * 100.0),
                0,
                processed,
                Some("ParallelChunked".to_string()),
                true,
    ...
    chunks.par_iter().try_for_each(|(offset, size)| -> Result<(), String> {
        ...
        #[cfg(windows)]
        {
            ...
            dst_file_write.seek_write(&buffer, *offset)
        ...
        #[cfg(not(windows))]
        {
            ...
            dst_file_write.write_all_at(&buffer, *offset)
                .map_err(|e| format!("Failed to write at offset {}: {}", offset, e))?;
        }
...
fn simd_buffered_copy(
    ...
    let _file_size = src_file.metadata()
        .map_err(|e| format!("Failed to get file metadata: {}", e))?
        .len();
    ...
    let simd_alignment = if hardware_info.has_avx2 { 32 } else if hardware_info.has_sse4 { 16 } else { 8 };
    ...
    loop {
        ...
        writer.write_all(&write_buffer[..bytes_read])
        ...
        let elapsed_secs = start_time.elapsed().as_secs_f64();
        let speed = if elapsed_secs > 0.0 { bytes_processed as f64 / elapsed_secs } else { 0.0 };
        
        progress_manager.update_file_progress(
            operation_id,
            format!("SIMD copy: {:.1} MB processed ({:.1} MB/s)", 
...
fn optimized_buffered_copy(
    ...
    let elapsed_secs = start_time.elapsed().as_secs_f64();
    let speed = if elapsed_secs > 0.0 { result as f64 / elapsed_secs } else { 0.0 };
...
pub async fn accelerated_copy_directory(
    ...
    thread::spawn(move || {
        let result = accelerated_copy_dir_impl(&source_clone, &destination_clone, &progress_manager_ref, &operation_id_clone);
...
fn accelerated_copy_dir_impl(
    ...
    progress_manager.update_file_progress(
        operation_id,
        format!("Copying {} files ({:.1} MB)", total_files, total_size as f64 / 1024.0 / 1024.0),
        0,
        0,
    ...
    file_operations.par_iter().try_for_each(|(src_path, dst_path, file_size)| -> Result<(), String> {
        let accelerated_ops = AcceleratedFileOps::new();
        let strategy = accelerated_ops.choose_copy_strategy(*file_size);
