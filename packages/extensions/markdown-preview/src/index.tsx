...
/**
 * SEC-04: Sanitize HTML output to prevent XSS attacks.
 *
 * Uses DOM-based sanitization for robust security:
 * - Removes <script>, <iframe>, <object>, <embed>, <form> elements
 * - Strips event handler attributes (onclick, onerror, onload, etc.)
 * - Neutralizes javascript: URLs in href/src attributes
 */
const sanitizeHtml = (html: string): string => {
  const div = document.createElement('div');
  div.innerHTML = html;

  // Remove dangerous elements (including SVG which can contain scripts)
  const dangerousElements = div.querySelectorAll('script, iframe, object, embed, form');
  dangerousElements.forEach((el) => el.remove());

  // Remove event handler attributes, javascript:/data: URLs, and encoded bypass attempts
  ...
  allElements.forEach((el) => {
    Array.from(el.attributes).forEach((attr) => {
      ...
      // Block all event handler attributes (onclick, onerror, onload, etc.)
      // Also catches SVG event handlers like onbegin, onrepeat
      ...
      if (/^\s*javascript\s*:/i.test(valueLower) || /^\s*data\s*:/i.test(valueLower)) {
        el.removeAttribute(attr.name);
        return;
      }

      // Block href/src attributes with encoded characters that could bypass filters
      if (nameLower === 'href' || nameLower === 'src' || nameLower === 'xlink:href') {
        if (valueLower.includes('&#') || valueLower.includes('%6a') || valueLower.includes('%4a')) {
          el.removeAttribute(attr.name);
          return;
        }
      }
