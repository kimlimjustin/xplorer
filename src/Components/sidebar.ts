...
const createSidebar = (): void => {
	const { ContextMenu } = require('./contextMenu'); //eslint-disable-line
	...
	const getFavoritesElement = (favorites: Favorites[]) => {
		...
		for (const favorite of favorites) {
			...
			favoritesElement += `<span data-listenOpen data-path = "${
				favorite.path
			}" data-isdir="${isdir}" class="sidebar-hover-effect sidebar-item"><img src="${getPreview(
				favorite.name,
        ...
        <div class="sidebar-hover-effect">
            <span class="sidebar-nav-item-dropdown-btn" data-section="favorites"><img src="${getPreview(
				'Favorites',
				'sidebar',
				false
	...
	const getDrivesElement = async () => {
		...
		// Return basic sidebar item element if there's no drives detected or its running on macOS
			...
			const result = `<div class="sidebar-nav-item ${
                ...
                <div class="sidebar-hover-effect">
                <span class="sidebar-nav-item-dropdown-btn" data-section="drives"><img src="${getPreview(
					'hard-disk',
					'favorites',
					false
